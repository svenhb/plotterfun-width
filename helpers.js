...
function pixelProcessor(config, imagePixels){
  ...
  return function(x, y) {
	...
	let a = imagePixels.data[4 * (pixIndex + 2)];
    if (contrast !== 0) {
  ...
  if (data[0][0].x) {
    for (let p in data) {
		for (let i=1;i<data[p].length;i++) 
			pathstring+=data[p][i-1].x.toFixed(2)+";"+data[p][i-1].y.toFixed(2)+";"+data[p][i].x.toFixed(2)+";"+data[p][i].y.toFixed(2)+";"+data[p][i].z.toFixed(2);
			postMessage(['svg-line',pathstring])
    }
  } else {
    for (let p in data) {
		for (let i=1;i<data[p].length;i++) 
		{	pathstring = data[p][i-1][0].toFixed(2)+";"+data[p][i-1][1].toFixed(2)+";"+data[p][i][0].toFixed(2)+";"+data[p][i][1].toFixed(2)+";"+data[p][i][2].toFixed(2);
			postMessage(['svg-line',pathstring])
		}		
    }
  }
